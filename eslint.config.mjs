...
export default [
...

	{
		...
		rules: {
			...
			'jsdoc/check-tag-names': ['error', { definedTags: ['spec', 'visual', 'e2e'] }],
		},
	},
	...
	{
		name: 'conduction/petstore-rules',
		rules: {
			// Fleet convention — scholiq, procest, pipelinq, decidesk and shillinq
			// all set this off. The remaining uses here are console.warn on a
			// failed fetch, i.e. diagnostics on an error path.
			'no-console': 'off',
	...
	{
		// Node CLI checkers: they print and exit by design.
		name: 'conduction/petstore-cli-checkers',
		files: [
